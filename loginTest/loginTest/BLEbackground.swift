...
/**
 ...
 6.キャラクタリスティックの受取
 */
class BLEbackground: UIViewController ,CBCentralManagerDelegate,CBPeripheralDelegate{
    var Timer:NSTimer!
    /** BLE機器のUUID配列 */
...
//    @IBOutlet weak var logView: UITextView!
    ...
    func centralManager(central: CBCentralManager, didDiscoverPeripheral peripheral: CBPeripheral, advertisementData: [String : AnyObject], RSSI: NSNumber) {
        ...
        if(peripheral.name != nil && peripheral.name!.containsString("iPass-")){
            ...
            myUUIDs.addObject(peripheral.identifier.UUIDString)
            //リストの最後のペリフェラルにつなげる(片っ端から)
            myCentralManager.connectPeripheral(self.appDelegate.myPeripheral.lastObject as! CBPeripheral, options: nil)
        }
    }
    /**
     Peripheralに接続
